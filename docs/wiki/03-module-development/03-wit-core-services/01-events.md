# ⚡ WIT Интерфейс `nms:core/events` — Гибридная Шина Сообщений

## 1. Назначение и архитектура шины событий

Интерфейс `nms:core/events` является фундаментальной транспортной магистралью микроядра платформы. Он обеспечивает асинхронную передачу сообщений между изолированными WASM-модулями, ядром и клиентским веб-интерфейсом через два независимых контура доставки:
