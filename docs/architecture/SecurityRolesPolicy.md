...
### ТК-5: Первичная настройка аккаунта: смена логина и пароля при первом входе
...
* **Then**: Интерфейс перехватывает ответ, запрещает доступ к дашборду и принудительно открывает диалог первичной настройки. Пользователь может 1 раз сменить свой логин (`username`) на постоянный и обязан установить новый пароль. После сохранения флаг `must_change_password` сбрасывается, и дальнейшая смена логина блокируется.
